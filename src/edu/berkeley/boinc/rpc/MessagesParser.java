	...
	public void parseMessages(String xml) {
		...
		try {
			while(pos < end) {
				/* skip spaces */
				while(pos < end) {
					if(!Character.isSpace(xml.charAt(pos))) break;
					pos++;
				}
				if(!inMsgs) {
					newPos = xml.indexOf("<msgs>");
					if(newPos == -1) throw new RuntimeException("Cant parse messages");
					pos = newPos + 6;
					inMsgs = true;
					...
					if(xml.startsWith("<project>", pos)) {
						...
						if(endTagIndex == -1) throw new RuntimeException("Cant parse messages");
