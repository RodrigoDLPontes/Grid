...
/**
 * GUI RPC - the way how GUI can manage BOINC core client and retrieve the
 * information from the client. This tries to be the same as original BOINC C++
 * GUI, but the names are rewritten for the sake of naming convention. Therefore
 * original RPC_CLIENT becomes RpcClient, get_cc_status() becomes getCcStatus()
 * etc.
 */
public class RpcClient {
	...
	private class Auth1Parser extends DefaultHandler {
		...
		public void endElement(String uri, String localName, String qName) throws SAXException {
			...
			if(qName.equalsIgnoreCase("nonce") && !mNonceParsed) {
	...
	private class Auth2Parser extends DefaultHandler {
		...
		public void endElement(String uri, String localName, String qName) throws SAXException {
			super.endElement(uri, localName, qName);
			if(qName.equalsIgnoreCase("authorized") && !mParsed) {
				mResult.append("authorized");
				mParsed = true;
			} else if(qName.equalsIgnoreCase("unauthorized") && !mParsed) {
	...
	/**
	 * Connect to BOINC core client via Unix Domain Socket (abstract,
	 * "boinc_socket")
	 * 
	 * @return true for success, false for failure
	 */
	public boolean open(String socketAddress, int port) {
		if(isConnected()) {
			// Already connected
			if(edu.berkeley.boinc.utils.Logging.LOGLEVEL <= 4) System.out.println("Attempt to connect when already connected");
			// We better close current connection and reconnect (address/port
			// could be different)
			close();
		}
		try {
			mSocket = new Socket();
			mSocket.connect(new InetSocketAddress(socketAddress, port), CONNECT_TIMEOUT);
			mSocket.setSoTimeout(READ_TIMEOUT);
			mInput = mSocket.getInputStream();
			mOutput = new OutputStreamWriter(mSocket.getOutputStream(), "ISO8859_1");
		} catch(IllegalArgumentException e) {
			if(edu.berkeley.boinc.utils.Logging.LOGLEVEL <= 4) System.out.println("connect failure: illegal argument");
			mSocket = null;
			return false;
		} catch(IOException e) {
			if(Logging.WARNING) System.out.println("connect failure");
			mSocket = null;
			return false;
		} catch(Exception e) {
			if(Logging.WARNING) System.out.println("connect failure");
		...
		if(Logging.DEBUG) System.out.println("Connected successfully");
		return true;
	}
	...
	public synchronized void close() {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("input close failure");
		}
		try {
			mOutput.close();
		} catch(IOException e) {
			if(Logging.WARNING) System.out.println("output close failure");
		}
		try {
			mSocket.close();
			if(Logging.DEBUG) System.out.println("close() - Socket closed");
		} catch(IOException e) {
			if(Logging.WARNING) System.out.println("socket close failure");
		}
		mSocket = null;
	}
	...
	/**
	 * Performs the BOINC authorization towards currently connected client. The
	 * authorization uses MD5 hash of client's password and random value.
	 * Clear-text password is never sent over network.
	 * 
	 * @param password
	 *             Clear text password used for authorization
	 * @return true for success, false for failure
	 */
	public synchronized boolean authorize(String password) {
		...
		try {
			...
			saxParser1.parse(new InputSource(new StringReader(auth1Rsp)), new Auth1Parser(mRequest));
			...
			saxParser2.parse(new InputSource(new StringReader(auth2Rsp)), new Auth2Parser(mRequest));
			if(!mRequest.toString().equals("authorized")) {
				if(Logging.DEBUG) System.out.println("authorize() - Failure");
				return false;
			}
		} catch(IOException e) {
			if(Logging.WARNING) System.out.println("error in authorize()");
			return false;
		} catch(SAXException e) {
			System.out.println("Malformed XML received in authorize()");
			return false;
		} catch(ParserConfigurationException e) {
			e.printStackTrace();
		}
		if(Logging.DEBUG) System.out.println("authorize() - Successful");
		return true;
	}
	...
	/**
	 * Checks whether current connection can be used (data can be sent and
	 * received) This is achieved by sending the empty BOINC command - so in
	 * case there is socket still opened, but other side already closed
	 * connection, it will be detected.
	 * 
	 * @return true if other side responds, false if data cannot be sent or
	 *         received
	 */
	 ...
	 *             The request itself
	 * @throws IOException
	 *              if error occurs when sending the request
	 */
	protected void sendRequest(String request) throws IOException {
		if(Logging.RPC_PERFORMANCE) if(Logging.DEBUG) System.out.println("mRequest.capacity() = " + mRequest.capacity());
		if(Logging.RPC_DATA) if(Logging.DEBUG) System.out.println("Sending request: \n" + request.toString());
	 ...
	 * @throws IOException
	 *              if error occurs when reading from socket
	 */
	protected String receiveReply() throws IOException {
		mResult.setLength(0);
		if(Logging.RPC_PERFORMANCE) if(Logging.DEBUG) System.out.println("mResult.capacity() = " + mResult.capacity());
		...
		int bytesRead;
		if(mInput == null) return mResult.toString(); // empty string
		do {
		...
		if(Logging.RPC_PERFORMANCE) {
			...
			if(Logging.DEBUG) System.out.println("Reading from socket took " + duration + " seconds, " + bytesCount + " bytes read (" + (bytesCount / duration) + " bytes/second)");
		}
		...
		if(Logging.RPC_DATA) {
			...
			try {
				while((dl = dbr.readLine()) != null) {
					++ln;
					if(Logging.DEBUG) System.out.println(String.format("%4d: %s", ln, dl));
				}
	...
	public synchronized VersionInfo exchangeVersions() {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in exchangeVersions()");
			return null;
		}
	}
	...
	public synchronized CcStatus getCcStatus() {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in getCcStatus()");
			e.printStackTrace();
			return null;
		}
	}
	...
	public synchronized ArrayList<Transfer> getFileTransfers() {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in getFileTransfers()");
			return null;
		}
	}
	...
	public synchronized HostInfo getHostInfo() {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in getHostInfo()");
			return null;
		}
	}
	
	/**
	 * Performs get_message_count RPC towards BOINC client Returns highest
	 * seqNo
	 * 
	 * @return result of RPC call in case of success, null otherwise
	 */
	public synchronized int getMessageCount() {
		...
		try {
			...
			if(Logging.DEBUG) System.out.println("RpcClient.getMessageCount returning: " + seqNo);
			return seqNo;
		} catch(IOException e) {
			if(Logging.WARNING) System.out.println("error in getMessageCount()");
			return -1;
		}
	}
	
	/**
	 * Performs get_messages RPC towards BOINC client Returns client messages
	 * that are more recent than seqNo param
	 * 
	 * @return result of RPC call in case of success, null otherwise
	 */
	public synchronized ArrayList<Message> getMessages(int seqNo) {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in getMessages()");
			return null;
		}
	}
	
	/**
	 * Performs get_notices PRC towards BOINC client Returns client messages
	 * that are more recent than seqNo param
	 * 
	...
	public synchronized ArrayList<Notice> getNotices(int seqNo) {
		...
		try {
			...
			if(notices == null) notices = new ArrayList<Notice>(); // do not
														// return
														// null
			return notices;
		} catch(IOException e) {
	...
	public synchronized ArrayList<Project> getProjectStatus() {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in getProjectStatus()");
			return null;
		}
	}
	...
	public synchronized ArrayList<Result> getActiveResults() {
		mLastErrorMessage = null;
		final String request = "<get_results>\n" + "<active_only>1</active_only>\n" + "</get_results>\n";
		try {
			sendRequest(request);
			ArrayList<Result> results = ResultsParser.parse(receiveReply());
			return results;
		} catch(IOException e) {
			if(Logging.WARNING) System.out.println("error in getActiveResults()");
			return null;
		}
	}
	...
	public synchronized ArrayList<Result> getResults() {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in getResults()");
			return null;
		}
	}
	...
	public synchronized CcState getState() {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in getState()");
			return null;
		}
	}
	
	/**
	 * Reports the current device state to the BOINC core client, if not called
	 * frequently, BOINC core client will suspend
	 * 
	 * @return true for success, false for failure
	 */
	public synchronized boolean reportDeviceStatus(DeviceStatusData deviceStatus) {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in networkAvailable()");
			return false;
		}
	}
	...
	/**
	 * Tells the BOINC core client that a network connection is available, and
	 * that it should do as much network activity as it can.
	 * 
	 * @return true for success, false for failure
	 */
		...
		try {
			...
			switch(operation) {
				...
				case PROJECT_RESET:
					...
					break;
				default:
					if(edu.berkeley.boinc.utils.Logging.LOGLEVEL <= 4) System.out.println("projectOp() - unsupported operation: " + operation);
					return false;
			}
			String request = "<" + opTag + ">\n" + "<project_url>" + projectUrl + "</project_url>\n" + "</" + opTag + ">\n";
			...
			if(Logging.WARNING) System.out.println("error in projectOp()");
			return false;
		}
	}
	...
	public synchronized boolean createAccount(AccountIn accountIn) {
		try {
			...
			if(accountIn.user_name != null) mRequest.append(accountIn.user_name);
			mRequest.append("</user_name>\n   <team_name>");
			if(accountIn.team_name != null) mRequest.append(accountIn.team_name);
			...
			if(Logging.WARNING) System.out.println("error in createAccount()");
			return false;
		}
	}
	...
	public synchronized AccountOut createAccountPoll() {
		try {
			...
			if(Logging.WARNING) System.out.println("error in getCreateAccountPoll()");
			return null;
		}
	}
	...
	public synchronized boolean lookupAccount(AccountIn accountIn) {
		try {
			...
			if(Logging.WARNING) System.out.println("error in lookupAccount()");
			return false;
		}
	}
	...
	public synchronized AccountOut lookupAccountPoll() {
		try {
			...
			if(Logging.WARNING) System.out.println("error in getLookupAccountPoll()");
			return null;
		}
	}
	...
	public synchronized boolean projectAttach(String url, String authenticator, String name) {
		try {
			...
			if(Logging.WARNING) System.out.println("error in projectAttach()");
			return false;
		}
	}
	...
	public synchronized ProjectAttachReply projectAttachPoll() {
		try {
			...
			if(Logging.WARNING) System.out.println("error in projectAttachPoll()");
			return null;
		}
	}
	
	/**
	 * performs acct_mgr_rpc towards client attaches account manager to client
	...
	public synchronized boolean acctMgrRPC(String url, String name, String passwd) {
		try {
			...
			if(Logging.WARNING) System.out.println("error in acctMgrRPC()");
			return false;
		}
	}
	
	/**
	 * performs acct_mgr_rpc with <use_config_file/> instead of login
	 * information
	 * 
	 * @return success
	 */
	...
	/**
	 * performs acct_mgr_rpc_poll towards client polls status of acct_mgr_rpc
	 * 
	...
	public synchronized AcctMgrRPCReply acctMgrRPCPoll() {
		try {
			...
			if(Logging.WARNING) System.out.println("error in acctMgrRPCPoll()");
			return null;
		}
	}
	...
	public synchronized AcctMgrInfo getAcctMgrInfo() {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in getAcctMgrInfo()");
			return null;
		}
	}
	...
	public synchronized boolean getProjectConfig(String url) {
		try {
			...
			if(Logging.WARNING) System.out.println("error in getProjectConfig()");
			return false;
		}
	}
	...
	public synchronized ProjectConfig getProjectConfigPoll() {
		try {
			...
			if(Logging.WARNING) System.out.println("error in getProjectConfigPoll()");
			return null;
		}
	}
	...
	public synchronized ArrayList<ProjectInfo> getAllProjectsList() {
		try {
			...
			if(Logging.WARNING) System.out.println("error in getAllProjectsList()");
			return null;
		}
	...
	/**
	 * Performs get_global_prefs_working RPC towards BOINC client
	 * 
	 * @return result of RPC call in case of success, null otherwise
	 */
	public synchronized GlobalPreferences getGlobalPrefsWorkingStruct() {
		try {
			...
			if(Logging.WARNING) System.out.println("error in globalPrefsWorking()");
			return null;
		}
	}
	...
	public synchronized boolean setGlobalPrefsOverride(String globalPrefs) {
		try {
			...
			if(Logging.WARNING) System.out.println("error in setGlobalPrefsOverride()");
			return false;
		}
	}
	...
	public synchronized boolean setGlobalPrefsOverrideStruct(GlobalPreferences globalPrefs) {
		try {
			...
			if(Logging.WARNING) System.out.println("error in setGlobalPrefsOverrideStruct()");
			return false;
		}
	}
	...
	public synchronized boolean quit() {
		try {
			...
			if(Logging.WARNING) System.out.println("error in quit()");
			return false;
		}
	}
	 ...
	 * 
	 * @param mode
	 *             1 = always, 2 = auto, 3 = never, 4 = restore
	 * @param duration
	 *             If duration is zero, mode is permanent. Otherwise revert to
	 *             last permanent mode after duration seconds elapse.
	 * @return true for success, false for failure
	 */
	public synchronized boolean setNetworkMode(int mode, double duration) {
		final String request = "<set_network_mode>\n" + modeName(mode) + "\n" + "<duration>" + duration + "</duration>\n" + "</set_network_mode>\n";
		try {
			sendRequest(request);
			SimpleReplyParser parser = SimpleReplyParser.parse(receiveReply());
			if(parser == null) return false;
			...
			if(Logging.WARNING) System.out.println("error in setNetworkMode()");
			return false;
		}
	}
	...
	public synchronized boolean setRunMode(int mode, double duration) {
		final String request = "<set_run_mode>\n" + modeName(mode) + "\n" + "<duration>" + duration + "</duration>\n" + "</set_run_mode>\n";
		try {
			sendRequest(request);
			SimpleReplyParser parser = SimpleReplyParser.parse(receiveReply());
			if(parser == null) return false;
			...
			if(Logging.WARNING) System.out.println("error in setRunMode()");
			return false;
		}
	}
	 ...
	 * @return true for success, false for failure
	 */
	public synchronized boolean setGpuMode(int mode, double duration) {
		final String request = "<set_gpu_mode>\n" + modeName(mode) + "\n" + "<duration>" + duration + "</duration>\n" + "</set_gpu_mode>\n";
		try {
			sendRequest(request);
			SimpleReplyParser parser = SimpleReplyParser.parse(receiveReply());
			if(parser == null) return false;
			...
			if(Logging.WARNING) System.out.println("error in setGpuMode()");
			return false;
		}
	}
		...
		try {
			...
			switch(operation) {
				...
				case TRANSFER_ABORT:
					...
					break;
				default:
					if(edu.berkeley.boinc.utils.Logging.LOGLEVEL <= 4) System.out.println("transferOp() - unsupported operation: " + operation);
					return false;
			}
			mRequest.setLength(0);
			mRequest.append("<");
			...
			if(Logging.WARNING) System.out.println("error in transferOp()");
			return false;
		}
	}
		...
		try {
			...
			switch(operation) {
				...
				case RESULT_ABORT:
					...
					break;
				default:
					if(edu.berkeley.boinc.utils.Logging.LOGLEVEL <= 4) System.out.println("resultOp() - unsupported operation: " + operation);
					return false;
			}
			mRequest.setLength(0);
			mRequest.append("<");
	...
	public synchronized boolean setCcConfig(String ccConfig) {
		final String request = "<set_cc_config>\n" + ccConfig + "\n</set_cc_config>\n";
		try {
			sendRequest(request);
			SimpleReplyParser parser = SimpleReplyParser.parse(receiveReply());
			if(parser == null) return false;
			...
			if(Logging.WARNING) System.out.println("error in setCcConfig()");
			return false;
		}
	...
	public synchronized String getCcConfig() {
		...
		try {
			...
			if(Logging.WARNING) System.out.println("error in getCcConfig()");
			return "";
		}
	}
	...
	public synchronized Boolean readCcConfig() {
		try {
			...
			if(Logging.WARNING) System.out.println("error in readCcConfig()");
			return false;
		}
	}
	...
	public synchronized Boolean runBenchmarks() {
		try {
			...
			if(Logging.WARNING) System.out.println("error in runBenchmark()");
			return false;
		}
